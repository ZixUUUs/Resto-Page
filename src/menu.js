...
const Menu = {
  getMenu: () => {
    ...
    divMenu.className = "myMenu";
    divMenu.id = "menuID";
    divContent.appendChild(divMenu);
  },
  Pizza: function (name, price) {
    this.name = name;
    this.price = price;
  },
  getCardMenu: () => {
    const pizzas = [
      new Menu.Pizza("Gc", "200"),
      new Menu.Pizza("Chimie", "300"),
      new Menu.Pizza("Physique", "300"),
      new Menu.Pizza("Bio", "500"),
      new Menu.Pizza("Geo", "500"),
      new Menu.Pizza("MaTh", "600"),
      new Menu.Pizza("Zinuxienne", "1000"),
    ];

    const divMenu = document.getElementById("menuID");

    pizzas.forEach((pizza, index) => {
      const cardMenu = document.createElement("div");
      cardMenu.className = "myCard";
      cardMenu.id = `card-${index}`;
      ...
      backside.innerHTML = `<svg id="pizzaFlip" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" height="800px" width="800px" version="1.1" id="Layer_1" viewBox="0 0 478.996 478.996" xml:space="preserve">
      ...
      pizzaPrice.textContent = `${pizza.price} DA`;
      frontside.appendChild(pizzaName);
      frontside.appendChild(pizzaPrice);
      cardMenu.appendChild(frontside);
      cardMenu.appendChild(backside);
